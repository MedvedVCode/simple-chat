...
const clearInput = () => {
	const interval = Math.round(500 / message.value.length);
...
const sendMessage = () => {
	...
	messageStore.addMessage(props.chatOwner, message.value);
	// message.value = '';
	clearInput();
};
</script>
...
.message {
	...
	border-radius: 0 0 $chatRadius $chatRadius;
	&-input {
		flex-grow: 2;
		padding: 10px 20px;
		border-radius: 20px;
